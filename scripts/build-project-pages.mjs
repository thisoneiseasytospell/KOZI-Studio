...
function validateProject(project, directoryName) {
  ...
  if (
    project.caseStudySlug !== undefined &&
    !/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(project.caseStudySlug)
  ) {
    fail(`${project.slug} caseStudySlug is not a URL-safe slug.`);
  }
  ...
  if (
    project.creditsHeading !== undefined &&
    (typeof project.creditsHeading !== "string" || !project.creditsHeading.trim())
  ) {
    fail(`${project.slug} creditsHeading must be a non-empty string.`);
  }
  ...
  if (
    ...
    (!Array.isArray(project.awards) ||
      project.awards.some(
        (award) => !award?.name?.trim() || !award?.href?.trim() || !award?.src?.trim() || !award?.alt?.trim()
      ))
  ) {
    fail(`${project.slug} awards must contain name, href, src, and alt values.`);
  }
  ...
  if (
    project.summaryLink !== undefined &&
    (!project.summaryLink?.label?.trim() || !project.summaryLink?.href?.trim())
  ) {
    fail(`${project.slug} summaryLink must contain label and href values.`);
  }

  if (
    project.recognition !== undefined &&
    (!Array.isArray(project.recognition) ||
      project.recognition.some(
        (item) => !item?.name?.trim() || !item?.href?.trim() || !item?.variant?.trim()
      ))
  ) {
    fail(`${project.slug} recognition must contain name, href, and variant values.`);
  }
  ...
  project.credits.forEach((credit, index) => {
    if (!credit?.name) {
      fail(`${project.slug} credit ${index + 1} needs a name.`);
    }

    if (credit.role !== undefined && typeof credit.role !== "string") {
      fail(`${project.slug} credit ${index + 1} role must be a string.`);
    }

    if (credit.href !== undefined && typeof credit.href !== "string") {
      fail(`${project.slug} credit ${index + 1} href must be a string.`);
    }
  });
  ...
  project.media.forEach((item, index) => {
    ...
    if (
      item.captionShowOn !== undefined &&
      !["all", "desktop", "mobile"].includes(item.captionShowOn)
    ) {
      fail(`${project.slug} media ${index + 1} captionShowOn must be all, desktop, or mobile.`);
    }
  });
}
...
async function loadProjects() {
  ...
  for (const directoryName of directories) {
    ...
    for (const [index, award] of (project.awards || []).entries()) {
      await assertAsset(award.src, `${project.slug} award ${index + 1}`);
    }
  ...
  projects.forEach((project) => {
    ...
    const target = projects.find((candidate) => candidate.slug === project.caseStudySlug);

    if (!target) {
      fail(`${project.slug} points to an unknown caseStudySlug: ${project.caseStudySlug}`);
    }

    if (target.caseStudySlug) {
      fail(`${project.slug} caseStudySlug must point directly to a canonical project.`);
    }
  });
...
function projectSummary(project) {
  const caseStudySlug = project.caseStudySlug || project.slug;
  ...
  return {
    ...
    detailPath: `/content/projects/${caseStudySlug}/project.json?v=7`,
    route: `/work/${caseStudySlug}/`,
  };
}

function createProjectPage(homepage, project, caseProject = project) {
  const title = `${escapeHtml(caseProject.title)} | KOZI Studio`;
  const description = caseProject.summary?.trim()
    ? escapeHtml(caseProject.summary.trim())
    : `Case study: ${escapeHtml(caseProject.title)} by Albert Kozikowski.`;
  const route = `https://www.kozi.studio/work/${caseProject.slug}/`;
  let page = homepage;
  ...
  page = page.replace(
    '<body id="top">',
    `<body id="top" data-initial-project="${escapeHtml(caseProject.slug)}">`
  );

  return `<!-- Generated by scripts/build-project-pages.mjs. -->\n${page}`;
}

function createWorkIndexPage(homepage) {
  const title = "Selected work | KOZI Studio";
  const description = "Selected work by Albert Kozikowski.";
  const route = "https://www.kozi.studio/work/";
  let page = homepage;
  ...
  page = page.replace(
    '<body id="top">',
    '<body id="top" data-initial-view="work">'
  );
...
async function main() {
  ...
  if (!checkOnly) {
    ...
    const workRouteDirectory = path.join(root, "work");
    await mkdir(workRouteDirectory, { recursive: true });
    await writeFile(
      path.join(workRouteDirectory, "index.html"),
      createWorkIndexPage(homepage)
    );

    for (const project of projects) {
      const caseProject = project.caseStudySlug
        ? projects.find((candidate) => candidate.slug === project.caseStudySlug)
        : project;
      const routeDirectory = path.join(root, "work", project.slug);
      await mkdir(routeDirectory, { recursive: true });
      await writeFile(
        path.join(routeDirectory, "index.html"),
        createProjectPage(homepage, project, caseProject)
      );
    }
  }
